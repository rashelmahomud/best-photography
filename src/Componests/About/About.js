import React from 'react';
import img from '../../images/doctor1.jpg'

const About = () => {
    return (
        <div className='container'>
            <h1>About Us</h1>
            <div><img src={img} alt="" /></div>
            <div>
                <h1>-DREAM FROM <br />
                    WEB DEVELOPMENT-</h1>
                    <p>Hello My Name is Rashel Mahomud. I have good dream for programing related work do. I can my life setup with web Development and computer sience related work todo its my big dreams. My favorite work programing and learn its best work i mena. I can my life a future all time this work with in my everythings.</p>
            </div>
        </div>
    );
};

export default About;